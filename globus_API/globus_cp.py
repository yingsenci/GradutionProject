...
def transfer(file_size):
    ...
    #no_encryption
    cmd = " "
    command = "globus-url-copy -vb" + cmd + "file:///mnt/disk2/" + file_name + " gsiftp://ubuntu/home/terry/ysc/"
    ...
    #encryption
    cmd = " -dcpriv "
    command = "globus-url-copy -vb" + cmd + "file:///mnt/disk2/" + "e" + file_name + " gsiftp://ubuntu/home/terry/ysc/"
    print "command: ", command
    fo.write("command: "+command+"\n")
    begin_time = time.time()
    process = os.popen(command)
...
if __name__ == "__main__":
    ...
    for size in range(file_size,file_size+30):
        if size == 0 :
            continue
        transfer(str(size)+"G")
